#define ANIM_CAT_FRAME_DURATION 100 // how long each frame lasts in ms
...
uint8_t ANIM_CAT_state = rdytoBONK;
...
static const char PROGMEM rdytoBONK_minimal[][ANIM_CAT_SIZE] =
{
    {
...
void eval_ANIM_CAT_state(void)
{
    ...
    switch (ANIM_CAT_state)
    {
        ...
        case BONK:
            if (!key_down) // BONK to rdytoBONK
            {
                ANIM_CAT_state = rdytoBONK;
            }
            break;
...
static void draw_bongo(bool minimal)
{
    ...
    {
        case rdytoBONK:
                oled_write_raw_P(rdytoBONK_minimal[0], ANIM_CAT_SIZE);
            break;
